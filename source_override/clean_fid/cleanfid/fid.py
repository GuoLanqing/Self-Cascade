...
def get_files_features(l_files, model=None, num_workers=12,
                       ...
                       custom_image_tranform=None, random_crop=False, 
                       n_crops_per_img=3):
    # wrap the images in a dataloader for parallelizing the resize operation
    dataset = ResizeDataset(l_files, fdir=fdir, mode=mode, random_crop=random_crop)
    ...
    if verbose:
        ...
        pbar = dataloader
    if random_crop:
        for i in range(n_crops_per_img):
            for batch in pbar:
                feat = get_batch_features(batch, model, device)
                if feat.size > feat.shape[0] * 2048:
                    feat = feat[:, :7, :, :].reshape(feat.shape[0], -1)
                    # print(f"after select privous 7 channels: {feat.size//feat.shape[0]}") # 2023
                l_feats.append(feat)
    else:
        for batch in pbar:
            feat = get_batch_features(batch, model, device)
            if feat.size > feat.shape[0] * 2048:
                feat = feat[:, :7, :, :].reshape(feat.shape[0], -1)
                # print(f"after select privous 7 channels: {feat.size//feat.shape[0]}")
            l_feats.append(feat)
...
def get_folder_features(fdir, model=None, num_workers=12, num=None,
                        ...
                        custom_image_tranform=None, random_crop=False, n_crops_per_img=3):
    ...
    np_feats = get_files_features(files, model, num_workers=num_workers,
                                  ...
                                  random_crop=random_crop, n_crops_per_img=n_crops_per_img)
    return np_feats
...
def fid_folder(fdir, dataset_name, dataset_res, dataset_split,
               ...
               random_crop=False, n_crops_per_img=3):
    ...
    np_feats = get_folder_features(fdir, model, num_workers=num_workers,
                                    ...
                                    custom_image_tranform=custom_image_tranform,
                                    custom_fn_resize=custom_fn_resize, random_crop=random_crop,
...
def compute_kid(fdir1=None, fdir2=None, gen=None,
            ...
            verbose=True, use_dataparallel=True, random_crop=False, 
            output_blocks=None, n_crops_per_img=3):
    # build the feature extractor based on the mode
    feat_model = build_feature_extractor(mode, device, use_dataparallel=use_dataparallel, output_blocks=output_blocks,)
...
def compute_fid(fdir1=None, fdir2=None, gen=None,
            ...
            random_crop=False, output_blocks=None, n_crops_per_img=3):
