...
def feature_extractor(name="torchscript_inception", device=torch.device("cuda"), resize_inside=False, use_dataparallel=True,
    ...
    if name == "torchscript_inception":
        ...
        model = InceptionV3(output_blocks=output_blocks, resize_input=False).to(device)
        model.eval()
        if use_dataparallel:
            model = torch.nn.DataParallel(model)
...
"""
def build_feature_extractor(mode, device=torch.device("cuda"), use_dataparallel=True, output_blocks=3):
    if len(output_blocks)>1 or output_blocks[0] != 3:
        print(f'output_blocks={output_blocks}')
        print("Use legacy_pytorch model")
